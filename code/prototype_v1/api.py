...
async def send_project_conversation_message(
    ...
    try:
        ...
        # Phase 3B: mirrors the ProjectExploreError clause above exactly. The conversation's own
        # VisualArtifact bridge (AssistantOrchestrator._run_visualize_bridge) already marks the
        # assistant turn FAILED with failure_category="visualize_failure" and re-raises the real
        # VisualArtifactError subtype (bridge resolution failure, store/provider failure, or
        # conflict) before this is reached - this only maps that same already-categorized failure to
        # a clean HTTP response, never exposing the raw internal exception.
        _raise_project_http_error(
