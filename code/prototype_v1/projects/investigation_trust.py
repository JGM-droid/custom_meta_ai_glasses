...
class ProjectInvestigationTrustService:
    ...
    def decide(self, project_id: str, session_id: str, request: ProjectTrustDecisionRequest) -> ProjectTrustDecisionResponse:
        session, retained, source, _ = self._load_source(project_id, session_id)
        ...
        if request.decision == ProjectTrustDecisionType.MORE_EVIDENCE:
            ...
            follow_up_id = follow_up.session_id
            metadata["follow_up_session_id"] = follow_up_id
        activity = self.activity_store.create_activity(project_id, ProjectActivityCreateRequest(
        ...
        if request.decision == ProjectTrustDecisionType.CONTINUE:
            project = self.activity_store.project_store.load_project(project_id)
            proposal = self.proposal_store.create_proposal(project_id, CheckpointProposalCreateRequest(
            ...
            metadata["checkpoint_proposal_id"] = proposal.proposal_id
            # Activities are immutable; proposal linkage is derived from its source_activity_ids.
        state = self.get_state(project_id, session_id)
