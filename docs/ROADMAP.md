...
- **STAGE 2 - Integrated Persistent Memory Shadow Slice** - **CURRENT MILESTONE.** One coherent shadow-memory system, not a set of unrelated mini-projects, implementing the Stage 1-proven design: durable facts/constraints with assertion modality and provenance; Decisions using subject+slot semantics with history-preserving supersession; Progress/events; deterministic current-truth eligibility filtering before any AI step; derived (not persisted) Current Project State with blocker preservation, resolved-area collapsing, and bounded AI salience selection for global continuation questions; targeted deterministic-first retrieval for scoped/topic-specific questions, unchanged from the prior gate. Records must be structured so actor/scope/time/provenance attribution (see the Product Requirement Amendment in `docs/PROJECT_MEMORY_ARCHITECTURE.md`) is possible later without redefining what a Project event is - reuse `ProjectActivity`'s existing `source_type`/provenance fields rather than a second attribution system. Shadow/log-only where canonical-mutation risk remains - this stage builds and inspects the integrated path, it does not make it authoritative yet. Lay the groundwork for the Editable Project Workspace requirement (natural-language corrections becoming authoritative Project events via interpretation/validation, never direct state edits, never erasing History - see `docs/PROJECT_MEMORY_ARCHITECTURE.md`) without implementing the correction/mutation behavior itself in this stage. No accounts/roles/permissions/collaboration UI in this stage.
...
- None blocking Stage 2. Stage 1's Current Project State/Salience question is resolved (PASSED, see `docs/PROJECT_MEMORY_ARCHITECTURE.md`'s Stage 1 Result) - the prior blocker on Stage 2 is cleared.
