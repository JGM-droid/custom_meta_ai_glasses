...
- **STAGE 3 - Visual Evidence Continuity** - **COMPLETE, IN PRODUCTION.** Durable Evidence descriptions (generated once per eligible Evidence, after the conversation turn's own lock releases), description-first retrieval for text-only visual questions, and selective original-image re-fetch only when the stored description is judged insufficient - all live in the real conversation path (not shadow-only, unlike Stage 2), since a two-tier retrieval decision has to shape the current turn's own reply. Verified via a real-backend, real-vision-model Living Room scenario proving both tiers with instrumented pixel-transmission evidence (Tier 1 sent zero image content; Tier 2 sent the correct original Evidence and correctly read a fine printed detail from it). See `docs/PROJECT_MEMORY_ARCHITECTURE.md`'s "Stage 3 Result" section for the full result, the narrow integration seam added to `AssistantRequest`/`OpenAIAssistantProvider`, and the one recorded reliability caveat (the AI selection call is not perfectly deterministic even at temperature 0 - its failure mode is safe abstention, never hallucination).
- **STAGE 4 - Real Conversation Integration** - **COMPLETE, IN PRODUCTION.** Stage 2's Structured Project Memory and Stage 1's derived Current Project State now feed normal `ProjectConversation` answers via a new question-aware retrieval seam (`projects/memory_retrieval.py`) merged into the existing Context Retriever's payload - deterministic-first subject/category/continuation matching, one bounded AI selection call only when ambiguous, current-vs-historical framing preserved with explicit CURRENT/HISTORY labeling. Verified via a real-backend 36-turn long-horizon Living Room scenario (leave/return simulated with a fresh orchestrator re-reading persisted state) that correctly answered all 13 required questions - budget, constraints, changed/original TV-stand decisions with grounded explanation, completed/open work, next action, a real blocker, both visual continuity tiers, and bounded context throughout (largest payload 9,908 characters, two-plus orders of magnitude below Stage 1's proven pathological range). One real correctness bug (a Stage 2 extraction slot-splitting ambiguity, previously only a recorded "known limitation") was found and fixed via a targeted extraction-prompt addition; a historical-rendering ordering issue was also fixed. See `docs/PROJECT_MEMORY_ARCHITECTURE.md`'s "Stage 4 Result" for full detail, exact provider-call counts, and one recorded non-blocking inefficiency (some declarative setup statements trigger an AI selection call that then correctly abstains). Route-file maintainability debt remains deferred - not addressed in this pass.
- **STAGE 5 - Realistic Usability Acceptance** - run the Persistent Project Usability Acceptance spec ("Living Room Long-Horizon Acceptance," `docs/PROJECT_MEMORY_ARCHITECTURE.md`) through the actual real application, not a toy unit test or a narrow provider eval. Extend the existing question set with cross-cutting retrieval questions that exercise Project History + Current Project State + targeted retrieval together, not just conversational recall - e.g. "What did I work on most recently?", "What have I completed?", "What changed?", "What have we done regarding the TV area?", "What remains in this part of the Project?". **COMPLETE / PASSED.** A first real dogfood run against a genuine pre-existing Project (predating Stage 2/3) found two BLOCKING issues - missing-memory fabrication (an unrelated known subject presented as the answer to a question with no durable memory coverage) and pre-Stage-3 Evidence false denial ("no photo was supplied" for a real photo with no stored description) - plus user-visible subject-identity drift on a natural progress correction. A bounded repair slice fixed all three (deterministic grounding check + honesty notice for missing memory; Evidence-exists recognition + forced original-pixel retrieval when undescribed; bounded known-subject-and-slot hints for extraction). An independent re-dogfood confirmed all three repairs held, and found one new HIGH finding: natural rewording ("is anything **actually** blocking me right now") could bypass Current Project State entirely because continuation/historical intent detection matched literal phrase substrings. Fixed with a token-sequence matcher tolerant of inserted filler words. A final short product-acceptance pass (blocker/continuation, leave-off, current-vs-historical decision, missing-memory honesty, and pre-Stage-3 visual Evidence, each through the real application with a fresh Project and natural wording not copied from any prior test) passed all five checks. See `docs/PROJECT_MEMORY_ARCHITECTURE.md`'s "Stage 5 Dogfood Result", "Stage 5 Repair Slice", "Stage 5 Independent Re-Dogfood", "Stage 5 Final Narrow Repair", and "Stage 5 Final Acceptance" sections for full detail and disclosed residual limitations (conversational self-consistency can still repeat a stale claim even after the memory layer correctly abstains; slot-reuse compliance during extraction is a real-provider judgment call, not fully deterministic - neither is a fabrication/safety violation). Not yet committed as of this writing.
...
- None blocking Stage 5. Stages 2, 3, and 4 are all complete (see `docs/PROJECT_MEMORY_ARCHITECTURE.md`'s "Stage 2 Result", "Stage 3 Result", and "Stage 4 Result") - no foundational assumption was falsified in any of them, so architecture exploration on the memory/context path remains closed.
